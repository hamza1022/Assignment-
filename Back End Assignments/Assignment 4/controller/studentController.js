...
const StudentModel= require('../models/students.model')
  ...
  const updateManyStudent = async (req, res, next) => {
    const newdata = req.body;
    const { message, data } = await StudentModel.updateMany(newdata);
    return res.status(201).json({
      message: message,
      data: data,
    });
  ...
  const deleteManyStudent = async (req, res, next) => {
    const { message } = await StudentModel.deleteMany();
    return res.status(201).json({
      message: message,
    });
  };
...
module.exports = {
    ...
    updateManyStudent:updateManyStudent,
